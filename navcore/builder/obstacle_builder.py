...
class ObstacleBuilder:
    ...
    def build_boundary(self):
        if self.config["boundary"]["gate"]["enabled"]:
            ...
            if edge_index == 0 or edge_index == 2:
                offset = np.random.uniform(0, self.env_config["arena"]["width"])
            else:
                offset = np.random.uniform(0, self.env_config["arena"]["height"] / 2)
            ...
            return boundaryWall.to_obstacle()
        ...
        for i in range(num_tables):
            ...
            if table_shape == "circular":
                ...
                table_center = Vector2(
                    self.rand.uniform(
                        -self.env_config["arena"]["width"] / 2 + table_radius,
                        self.env_config["arena"]["width"] / 2 - table_radius,
                    ),
                    self.rand.uniform(
                        -self.env_config["arena"]["height"] / 2 + table_radius,
                        self.env_config["arena"]["height"] / 2 - table_radius,
                    ),
                )
                ...
                table_center = Vector2(
                    self.rand.uniform(
                        -self.env_config["arena"]["width"] / 2 + table_width / 2,
                        self.env_config["arena"]["width"] / 2 - table_width / 2,
                    ),
                    self.rand.uniform(
                        -self.env_config["arena"]["height"] / 2 + table_height / 2,
                        self.env_config["arena"]["height"] / 2 - table_height / 2,
                    ),
                )
